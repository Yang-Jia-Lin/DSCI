...
def plot_bubble_chart(df: pd.DataFrame, save_dir=Path(RESULT_BASELINE_PATH)):
    ...
    df['display_name'] = df['name'].map(lambda x: name_map.get(x, x))

    # 计算气泡大小
    obj_min, obj_max = df['objective'].min(), df['objective'].max()
    if obj_max != obj_min:
        # 将基础大小从 150 提至 200，映射极差提至 1500，气泡会更饱满
        df['bubble_size'] = 200 + (df['objective'] - obj_min) / (obj_max - obj_min + 1e-6) * 1500
    else:
        df['bubble_size'] = 1000
    df_sorted = df.sort_values('latency_ms').reset_index(drop=True)
    ...
    if save_dir:
        ...
        file_name = f"baseline_bubble_chart_{datetime.now().strftime('%H%M')}"
...
def plot_utility_bar(df: pd.DataFrame, save_dir=Path(RESULT_BASELINE_PATH)):
    ...
    df['display_name'] = df['name'].map(lambda x: name_map.get(x, x))
    bars = ax2.bar(df['display_name'], df['objective'], color=colors[:len(df)], width=0.7)

    for bar in bars:
        h = bar.get_height()
        # 标注位置处理：如果 utility 是负的，文字标在柱子下方
        va = 'bottom' if h > 0 else 'top'
        offset = 0.02 * df['objective'].max() if h > 0 else -0.05 * abs(df['objective'].min())
        ax2.text(bar.get_x() + bar.get_width() / 2., h + offset,
                 f'{h:.2f}', ha='center', va=va)
    ax2.set_ylabel('Total Utility')

    y_min = min(0, df['objective'].min() * 1.2)
    y_max = max(0, df['objective'].max() * 1.2)
    ...
    if save_dir:
        save_dir.mkdir(parents=True, exist_ok=True)
        file_name = f"baseline_bar_chart_{datetime.now().strftime('%m%d_%H%M')}"
        save_path = save_dir / file_name
        save_fig_for_ieee(save_path)
