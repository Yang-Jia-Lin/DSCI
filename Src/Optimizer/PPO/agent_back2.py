        ...
        for epoch in range(self.hparams['max_epochs']):
            ...
            while steps < self.hparams['target_steps']:
                ...
                if np.isnan(reward) or np.isinf(reward):
                    print(f"[Warning] Got invalid reward at step {steps}, replaced with -1e6")
                    reward = -1e6
            ...
            if len(history) > patience:
                ...
                if np.std(recent_window) < tolerance:
                    print(
                        f"[Early Stop] PPO converged at epoch {epoch} with recent rewards std: {np.std(recent_window):.5f}")
                    break
