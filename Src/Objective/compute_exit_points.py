...
if __name__ == "__main__":
    ...
    # 错误原因分析
    # --------------------------------------------------------
    print(f"{'Case Type':<15} | {'X Vector':<20} | {'Result (c0, c1)':<18} | {'Local':<10} {'Edge':<15} {'Cloud':<10}")
    print("-" * 95)
    ...
    print(f"{'0 Cut (Local)':<15} | {str(x_case0):<20} | {f'({c0}, {c1})':<18} | {loc:<10} {edg:<15} {cld:<10}")
    ...
    loc, edg, cld = interpret_ranges(c0, c1, m)
    print(f"{'2 Cuts (L-E-C)':<15} | {str(x_case2):<20} | {f'({c0}, {c1})':<18} | {loc:<10} {edg:<15} {cld:<10}")
    ...
    if c1 == -1:
        print("  当前返回 (-1) 会导致 compute_latency 中的 range(cut0, cut1) 变为空 range(2, -1)。")
    ...
    test_cases = [
        ...
        # --- 1次切分 (Sum=1) 的边界测试 ---
        ("1. 极早切分 (第0层后)", np.array([1, 0, 0, 0, 0, 0])),  # Local做完Layer0就扔给Edge
        ("2. 中间切分", np.array([0, 0, 1, 0, 0, 0])),
        ("3. 极晚切分 (倒数第2层)", np.array([0, 0, 0, 0, 1, 0])),
        ("4. 最后切分 (跑完全部)", np.array([0, 0, 0, 0, 0, 1])),  # Local做完所有，传给Edge结果?

        # --- 2次切分 (Sum=2) 的边界测试 ---
        ("5. 连续切分 (0, 1)", np.array([1, 1, 0, 0, 0, 0])),  # Local做0, Edge做1, Cloud做剩下
        ("6. 连续切分 (中间)", np.array([0, 0, 1, 1, 0, 0])),
        ("7. 极限首尾 (0, 5)", np.array([1, 0, 0, 0, 0, 1])),  # Local做0, Cloud做无? Edge包圆?
        ("8. 间隔切分", np.array([0, 1, 0, 0, 1, 0])),
    ]

    print(f"{'ID':<3} | {'Description':<20} | {'X Vector':<20} | {'Result (c0, c1)':<15} | {'Logic Check'}")
    print("-" * 90)
